"""Load the GS electricity price forecasting dataset."""
...
RAW_DIR = Path("data/raw/gs_price")
PROCESSED_PATH = Path("data/processed/gs_price.pkl")
RAW_DIR_2025 = Path("data/raw/gs_price_2025")
PROCESSED_PATH_2025 = Path("data/processed/gs_price_2025.pkl")
TRAIN_ONLY_NORMALIZATION = "train_only"
FULL_DATA_NORMALIZATION = "full_data"
PRICE_BOUNDS_NORMALIZATION = "price_bounds"
...
def _normalise_occupancy(
    ...
    *,
    ...
    if normalization_source == PRICE_BOUNDS_NORMALIZATION:
        source = None
...
def load_occupancy(raw_dir: Path = RAW_DIR) -> pd.DataFrame:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"GS price matrix not found: {path}. "
            "Run `python scripts/prepare_gs_price.py` first."
        )
...
def load_weather(raw_dir: Path = RAW_DIR, timestamps=None) -> pd.DataFrame:
    ...
    if timestamps is not None:
        weather = weather.reindex(pd.DatetimeIndex(timestamps)).interpolate(method="time", limit_direction="both").ffill().bfill()
    return weather
...
def load_price(raw_dir: Path = RAW_DIR, timestamps=None) -> pd.DataFrame:
    ...
    if timestamps is not None:
        price = price.reindex(pd.DatetimeIndex(timestamps)).interpolate(method="time", limit_direction="both").ffill().bfill()
    return price
...
def load_adjacency(raw_dir: Path = RAW_DIR, n_nodes: int | None = None, node_ids: list[str] | None = None) -> np.ndarray:
    path = raw_dir / "adjacency.csv"
    if not path.exists():
        return np.zeros((n_nodes or 1, n_nodes or 1), dtype=np.float32)
    df = _normalise_columns(pd.read_csv(path))
...
def load_gs_price(
    ...
    print(
        f"[GS-Price] T={occ_raw.shape[0]}, N={occ_raw.shape[1]}, "
        f"raw range=[{vmin:.3f}, {vmax:.3f}], normalization={normalization_source}"
    )
    return result


def load_gs_price_2025(
    raw_dir: Path = RAW_DIR_2025,
    processed_path: Path | None = None,
    force_reprocess: bool = False,
    normalization_source: str = PRICE_BOUNDS_NORMALIZATION,
) -> dict:
    return load_gs_price(
        raw_dir=raw_dir,
...
if __name__ == "__main__":
    data = load_gs_price(force_reprocess=True)
